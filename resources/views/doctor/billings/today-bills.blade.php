...
@section('content')
    <div class="p-6">
           <div class="flex flex-col gap-4 mb-4 lg:flex-row lg:justify-between lg:items-center">
            <h2 class="inline_block text-2xl font-bold"> {{ __('messages.todays_bills') }} </h2>
           <div class="flex flex-col gap-2 lg:flex-row lg:items-center">
            {{-- this si for search bar --}}
            <div class="flex flex-col gap-2 sm:flex-row">
            <form method="GET"
             action="{{ route('doctor.today.bills') }}"
         ...
         <a href="{{ route('doctor.today.bills') }}"
            class="px-4 py-2 text-white bg-gray-700 rounded">
             {{ __('messages.refresh') }}
         </a>
          <div class="flex flex-wrap gap-2">
          <a href="{{ route('doctor.all-bills') }}"
                class="px-4 py-2 text-white bg-blue-600 rounded">{{ __('messages.view_all_bills') }}</a>
